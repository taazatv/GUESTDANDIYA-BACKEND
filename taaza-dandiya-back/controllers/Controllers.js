...
exports.uploadUserInfo = async (req, res) => {
  ...
  try {
    // Event date ko parse karo
    const eventDay = new Date(eventDate).getDate().toString().padStart(2, "0"); 
    // ex: "29" ya "05"

    // Token generate: first char + eventDay + rest coupon + last 4 phone
    const token = coupon[0] + eventDay + coupon.slice(1) + phone.slice(-4);

    // Booking date (user ne kis din register kiya)
    ...
    const smsUrl = `http://web.poweredsms.com/submitsms.jsp?user=TAZATV&key=44426475efXX&mobile=${encodeURIComponent(
      ...
      `Confirmed! Booking ID ${token}. You are entitled to ${1} tickets dated ${eventDay} for Taaza Dandiya @Netaji Indoor Stadium subject to clearance of payment. T%26C apply. Go to the Ticket counter at venue to redeem. -TaazaTv`
